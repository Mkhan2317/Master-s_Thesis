...
for current_date in tqdm(date_range):
    ...
    # Save raw data
    data.to_csv(f'market_data/data_{month_start}_{month_end}.csv')
    
    # Process returns
    returns = data.pct_change().iloc[1:]
    returns.to_csv(f'market_data/returns_{month_start}_{month_end}.csv')
    ...
    with open(f'llm_views/{model_name}_{month_start}_{month_end}.json', 'w') as f:
