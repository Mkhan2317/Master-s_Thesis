...
def calculate_llm_returns(tau = 0.025):
    # Read LLM results
    llama_results = pd.read_csv(f'evaluation_results/llama_black_litterman_weights_tau_{tau}.csv')
    qwen_results = pd.read_csv(f'evaluation_results/qwen_black_litterman_weights_tau_{tau}.csv')
    gemma_results = pd.read_csv(f'evaluation_results/gemma_black_litterman_weights_tau_{tau}.csv')
    ...
    for month in range(6, 12):  # Now only going up to November since December weights will be for January
        ...
        future_data = pd.read_csv(f'market_data/returns_{returns_start}_{returns_end}.csv')
...
if __name__ == "__main__":
    ...
    # save to csv
    llama_returns.to_csv(f'evaluation_results/llama_black_litterman_returns_tau_{tau}.csv', index=False)
    qwen_returns.to_csv(f'evaluation_results/qwen_black_litterman_returns_tau_{tau}.csv', index=False)
    gemma_returns.to_csv(f'evaluation_results/gemma_black_litterman_returns_tau_{tau}.csv', index=False)