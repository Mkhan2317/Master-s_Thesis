...
def process_period(start_date, end_date, tau):
    ...
    # Load common stocks
    with open('llm_views/common_stocks.json', 'r') as f:
    ...
    # Load returns data
    returns = pd.read_csv(f'market_data/returns_{start_date}_{end_date}.csv', index_col=0)
    ...
    # Load LLM responses
    # with open(f'llm_views/llama_{start_date}_{end_date}.json', 'r') as f:
    #     llama_dict = json.load(f)
    # with open(f'llm_views/qwen_{start_date}_{end_date}.json', 'r') as f:
    #     qwen_dict = json.load(f)
    with open(f'llm_views/gemma_{start_date}_{end_date}.json', 'r') as f:
...
def main():
    ...
    # llama_results_df.to_csv(f'evaluation_results/llama_black_litterman_weights_tau_{tau}.csv', index=False)
    # qwen_results_df.to_csv(f'evaluation_results/qwen_black_litterman_weights_tau_{tau}.csv', index=False)
    gemma_results_df.to_csv(f'evaluation_results/gemma_black_litterman_weights_tau_{tau}.csv', index=False)
